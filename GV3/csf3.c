...
static uint64_t inline decode(const csf_t * const csf, const uint64_t value) {	
	for (int curr = 0;; curr++)
...
static uint64_t inline get_value(const uint64_t * const array, uint64_t pos, const int width) {
	...
	if (start_bit <= l) return array[start_word] << (l - start_bit) >> l;/*WARNING Added parentheses for greater clarity **(l - start_bit)** */
	return array[start_word] >> start_bit | array[start_word + 1] << (64 + l - start_bit) >> l;/*WARNING Added parentheses for greater clarity **(64 + l - start_bit)** */
}
